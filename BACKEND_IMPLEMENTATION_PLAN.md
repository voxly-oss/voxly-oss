...
### 9.8 Milestone 5 pre-implementation audit — GitHub Context — 2026-07-26

§9.4 said GitHub context loading is "real, no fabrication found" — still true, but that pass only looked at `messaging_core.py`'s consumption side. A deeper look before Milestone 5, specifically checking for duplication (per that milestone's explicit "do not duplicate repository data" requirement), found a real gap the original audit missed:

**PH3-TD-03 (new): two independent GitHub-fetch paths that never share data.**
- **Path A** (Phase 2 Milestone 2, `ProjectResponse.github_stats`): reads the `github_cache` DB table directly. That table is populated exclusively by `tasks/github_sync.py`'s `sync_single_repo` Celery task, which runs hourly via Celery Beat (`sync_all_github_repos`) and calls `github_service.fetch_github_stats()`.
- **Path B** (`messaging_core.py`'s AI-context loading, via `cache_service.get_github_stats_cached()`): checks a **separate** Redis-or-in-memory cache keyed `github:stats:{project_id}`, 1-hour TTL. On a cache miss it calls the **same** `github_service.fetch_github_stats()` function directly — an independent live GitHub API call that never reads or writes the `github_cache` table Path A uses.

Consequence: the dashboard (Path A) and what the AI tells a client (Path B) can show **different** commit/issue/PR numbers at the same moment, since each path refreshes independently (Path A hourly via Celery, Path B on its own rolling 1-hour-since-last-miss window). Both hit the real GitHub API for the same repo, independently, doubling real API call volume for no benefit. Confirmed via code, not guessed: `sync_single_repo` explicitly calls `invalidate_github_cache(project_id)` after writing to `github_cache` — meaning whoever wrote Path A's sync job was aware a Redis cache existed and tried to keep it in sync, but Path B's cache key/value was never actually sourced from the table that invalidation call is meant to protect.

**Not fixed this milestone** — "reuse the existing pipeline, do not redesign the GitHub integration" is an explicit constraint, and consolidating these two paths is a real design decision (which one becomes canonical, whether Path B should read `github_cache` instead of its own cache, whether the 1-hour Celery cadence is fast enough for conversational use) that deserves its own scoped pass, not a fix folded into an unrelated milestone. Tracked here for whenever that's prioritized.

**What Milestone 5 ships instead:** conversation-level GitHub context exposure that deliberately reuses Path A (`github_cache`, via the already-defined `GitHubStatsSchema` from `app/schemas/project.py`) rather than adding a third path — see the Milestone 5 report for exact endpoints. No new fetch, no new cache, no new table.

Also confirmed, not previously stated explicitly: **`github_cache` has no "build status" or "deployment status" column, and no pipeline anywhere computes either.** The original Phase 3 kickoff's "never fabricate: build status, PR counts, deployment status" is satisfied by construction — PR counts are real (`pull_requests` column), build/deployment status are simply not exposed anywhere, because there is nothing real to expose (no CI/CD integration exists in this codebase today).
