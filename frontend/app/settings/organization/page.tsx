...
import { Code2, MessageSquare, Mail, Phone } from 'lucide-react';
import SettingsShell from '@/components/SettingsShell';
import { Panel, PanelRow, PanelText } from '@/components/SidePanel';

// This app has no multi-tenant Organization model yet (single account per
// user) — most of this page is illustrative, matching the design's mock
// workspace. The few real data points (owner identity, client/project
// counts) are wired to the authenticated user and live API data.
            ...
            <div className="flex flex-col xl:flex-row gap-6 items-start">
                <div className="flex-1 min-w-0 w-full flex flex-col gap-[18px]">
                    ...
                    <div className="flex items-start justify-between gap-4 flex-wrap">
                        ...
                        <Button className="bg-primary hover:bg-primary/90 text-primary-foreground font-semibold text-[13px] rounded-lg px-4 py-[9px] h-auto">Upgrade plan</Button>
                    </div>
                    ...
                    <div className="flex gap-5 flex-wrap px-[18px] py-3.5 border border-border rounded-xl bg-card">
                        <div><span className="font-display font-bold text-[17px] text-foreground">1</span><span className="text-[11.5px] text-voxly-ink-5 ml-1.5">member</span></div>
                    ...
                    <div className="border border-border rounded-[14px] bg-card overflow-hidden">
                        {[
                            { icon: Code2, name: 'GitHub', status: clients.length > 0 ? 'connected' : 'not connected', on: true },
                            { icon: MessageSquare, name: 'WhatsApp', status: `${clients.filter((c: any) => c.phone).length} ${clients.filter((c: any) => c.phone).length === 1 ? 'number' : 'numbers'} connected`, on: clients.some((c: any) => c.phone) },
                            { icon: Mail, name: 'Email', status: `${clients.filter((c: any) => c.email).length} ${clients.filter((c: any) => c.email).length === 1 ? 'inbox' : 'inboxes'} connected`, on: clients.some((c: any) => c.email) },
                            { icon: Phone, name: 'Voice', status: 'not connected', on: false },
                        ].map((svc, i, arr) => (
                            <div key={svc.name} className={`flex items-center gap-3 px-[18px] py-3 ${i < arr.length - 1 ? 'border-b border-border' : ''}`}>
                                ...
                                <span className={`w-1.5 h-1.5 rounded-full flex-none ${svc.on ? 'bg-voxly-success' : 'bg-voxly-heat'}`} />
                                <span className={`text-[12px] w-[170px] text-right ${svc.on ? 'text-voxly-ink-6' : 'text-voxly-heat'}`}>{svc.status}</span>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="w-full xl:w-80 flex-none flex flex-col gap-3.5">
                    ...
                    <Panel title="Recent Activity" defaultOpen={false}>
                        <PanelText>No recent activity.</PanelText>
                    </Panel>
                </div>
            </div>
        </SettingsShell>
